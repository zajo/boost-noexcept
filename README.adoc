...
Alternatively, they can be built using link:http://mesonbuild.com[Meson]. In this case, both Boost and Lua (needed for one of the examples) will be downloaded automatically under the `subprojects` directory. Use `mesonconf` to select the build configuration (multiple build options are available).

...
NOTE: For a comparison between various error-reporting libraries and proposals, see <<alternatives,Noexcept alternatives>>.
...
The following table summarizes the key differences between the different error handling libraries; see <<design_rationale,Design rationale>> to understand the motivation for the Noexcept design:

.Comparison between error handling libraries
[cols="6,^.^2,^.^2,^.^2,^.^2,^.^2"]
|===
| |1|2|3|4|5

| Requires specific return type in functions that could potentially fail
| <<example_FILE,no>>
| yes
| yes
| yes
...
| Functions must enumerate the possible failures
| no
| yes
| yes
| no
...
| User-defined error types supported
| <<example_lua,yes>>
| yes
| yes
| yes (dynamic allocation)
| yes

| Errors may be reported from C callbacks
| <<example_lua,yes>>
| no
| no
| no
...
| Reported errors may cross programming language boundaries
| <<example_lua,yes>>
| no
| no
| no
...
| Errors may be moved to a different thread
| <<example_thread,yes>>
| yes
| yes
| yes
...
| Strict value-or-error semantics
| <<example_optional,optional>>
| yes
| yes
| no
| no

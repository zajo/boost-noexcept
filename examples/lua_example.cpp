...
std::shared_ptr<lua_State> init_lua_state() {  //<1>
...
int main() {
    ...
    for( int i=0; i!=10; ++i ) {
        if( auto tr=try_(call_lua(&*L)) )
            std::cout << "do_work succeeded, answer=" << tr.get() << '\n';
        else if( auto err=tr.catch_<do_work_error>() )  //<5>

...
WARNING: If unhandled errors remain at the time the current thread terminates, Noexcept calls `std::terminate()`. Use `catch_<>` to handle any error regardless of its type.
...
+
NOTE: If control is entering a scope where exception handling is enabled, you can convert Noexcept errors to exceptions by `try_(f()).result()`, which throws on error.

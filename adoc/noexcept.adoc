...
The passed `error` object (of user-defined type) becomes the current error for the calling thread. The error remains current until captured by a `try_`. For threads that have a current error, `has_current_error()` returns true.
...
_Preconditions:_:: `!has_current_error()`
...
If `throw_` was used to set the current error for the calling thread, and if that error has not been captured by a `try_`, `has_current_error` returns `true`. Otherwise it returns `false`.
...
The `try_`  function template is used in contexts that handle errors reported (using `throw_`) by a lower level function: if the return value of the function is of type `R`, it is moved into the `handler<R>` object returned by `try_`.

The `handler` object then uses `result_traits<R>::succeeded(res)` to check for success. If it returns `true`, no further action is taken; otherwise the current error for the calling thread is moved into `*this`, leaving it with `!has_current_error()`. The `catch_` member function can then be used to check for specific error types to handle.
...
_Preconditions:_:: `result_traits<R>::succeeded(res) == !has_current_error()`.
...
~handler() noexcept;
----
_Preconditions:_:: `!has_current_error() || bool(*this)`, or else the program is ill-formed.
...
NOTE: Errors captured by `handler` objects are marked as handled when `catch_<E>` is called, but only if it can be converted to `E *`.
...
R & result();
----
_Effects:_:: If `bool(*this)` is `true`, `result()` returns a reference to the result object passed to `try_` and moved into `*this`. Otherwise calling `result()` throws the error as a C++ exception.
...
E * catch_() noexcept;
----
_Preconditions:_:: `!(*this)`
_Returns:_:: `dynamic_cast<E *>(e)`, where `e` is a pointer to the error object captured into `*this`.
_Effects:_:: If the cast is successful, the error object captured into `*this` is marked as `handled`. See <<_handler,`~handler()`>>.
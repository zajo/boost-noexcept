...
Functions which could fail (except if they use exception handling) must communicate the failure directly to the caller -- but they also need to be able to return a value in case of success.
...
NOTE: Because translation between different error reporting mechanisms in large programs is both imperfect and prone to errors (and because error handling code is very difficult to test), such traslation should be kept to the absolute minimum. Ideally, the same object created when the error is first reported should reach the error handling code intact.
...
NOTE: Noexcept does define a `<<try_,result<T> >>` template, which can be used to capture the result of a function call (value-or-error) in case its consumption must be postponed, for example if it has to be transported to a different thread; see <<programming_techniques,Programming techniques and guidelines>>.
...
----
BOOST_NOEXCEPT_USE_BOOST_TSS
----
By default, Noexcept uses the standard `thread_local` macro for thread-local storage. If `BOOST_NOEXCEPT_USE_BOOST_TSS` is `#defined`, Noexcept uses link:http://www.boost.org/doc/libs/release/doc/html/thread/thread_local_storage.html[`boost::thread_specific_ptr`] instead.
...
link:https://ned14.github.io/outcome/[Outcome] is another error handling library (which recently underwent a formal Boost review). It is similar to Noexcept in that it does not use exceptions, but it takes an alternative design approach.

The main difference is that Outcome transports errors up the call chain one level at a time: functions are required to return `outcome<T>` which can store a value in case of success -- and there are several possibilities for transporting different error objects in case of a failure. In contrast, Noexcept does not impose a special return type; see <<design_rationale,Design rationale>>.
...
[qanda]
Does Noexcept allocate memory dynamically?:: Generally no, except for very large objects passed to `throw_`. In addition, if using `boost::thread_specific_ptr` as the TLS provider (see <<configuration,Macros and configuration>>), it will perform a single allocation only the very first time `throw_` is used in a given thread.
...
The unit tests can be build within the Boost framework: clone Noexcept under the `libs` subdirectory in your boost installation, then `cd` into `noexcept/test` and execute `b2` as usual.

Alternatively, the unit tests can be built using link:http://mesonbuild.com[Meson]. In this case, both Boost and Lua (needed for one of the examples) will be downloaded automatically under the `subprojects` directory. Use `mesonconf` to select the build configuration (multiple build options are available).

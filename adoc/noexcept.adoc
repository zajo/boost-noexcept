...
NOTE: At the time of this writing, the standard `thread_local` is not yet available on some platforms or may not be used with non-trivial types.
...
The main difference is that Outcome transports errors up the call chain one level at a time: functions are required to return `outcome<T>` which may store a value in case of success -- and there are several possibilities for transporting different error objects in case of a failure. In contrast, Noexcept does not impose a special return type; see <<design_rationale,Design rationale>>.

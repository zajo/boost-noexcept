...
If `<<throw_,throw_>>` was used to set the current error for the calling thread, and if that error has not been captured by an `<<error_handler,error_handler>>` (or a `try_handler`, see `<<try_,try_>>`), `has_current_error` returns `true`. Otherwise it returns `false`.
